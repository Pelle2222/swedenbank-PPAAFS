...
module.exports = function(){
    ...
    this.Given(/^I have logged as Johan$/, async function () {
        ...
        await helpers.loadPage('http://localhost:3000/#login');
         ...
         await driver.findElement(By.id("username")).sendKeys("Johan");   
      ...
      this.When(/^I choose option of Postgiro$/, async function () {
        await sleep(1000);
        await driver.findElement(By.xpath("//*[@id=\"accountTypes\"]/label[2]/input")).click(); //radio buttons
        await sleep(1000); 
      });
      ...
      this.When(/^I enter account number (\d+)\-(\d+) to transfer$/, async function (arg1, arg2) {
        ...
        await sleep(1000);
        await driver.findElement({id: 'toAccountNumber'}).sendKeys('123456-9');
        await sleep(1000);
      });
      ...
      this.When(/^I enter the amount of (\d+) kronor to transfer$/, async function (arg1) {
        ...
        await sleep(1000);
        await driver.findElement({id: 'sum'}).sendKeys('1000');
        
      });

      this.When(/^I write a message\-Telia (\d+) kronor phone bill$/, async function(arg1) {
        await sleep(1000);
        await driver.findElement(By.id("label")).sendKeys("Telia 1000 kronor phone bill"); 
      ...
      this.Then(/^I verify if the money is transferred from Johans account to Telia$/, async function () {
        ...
        assert.equal(balance, '-1000', 'THE AMOUNT JOHAN TRANSFERED SHOULD BE: ' +expectedAmount+' AND IS: ' +balance); 
